...
// 是否打包成外部lib包
let outdist = process.env.outdist;
module.exports = {
    //基本路径
    publicPath: "./",
    //输出文件目录
            ...
            config.plugin("uglifyjs-plugin").use("uglifyjs-webpack-plugin", [
                {
                    uglifyOptions: {
                        warnings: false,
                        compress: {
                            // 这里会导致生产上console.error无效 所以看不到js报错信息 调试的时候需要注意
                            drop_console: true,
                            drop_debugger: false,
    ...
    configureWebpack: (config) => {
        ...
        if (isProd && process.env.REPORT === "report") {
        ...
        if (!isProd) {
            ...
            config.plugins.push(
                new StyleLintPlugin({
                    files: ["src/**/*.{vue,html,scss,sass,less}"],
                    failOnError: false,
                    cache: true,
                    fix: Boolean(process.env.CSSFIX)
                })
            );
    ...
    pluginOptions: outdist ? {} : require("./dll.config")
};
