...
export default class ConfirmBox extends View {
    ...
    public zqData = [
        [950.2	,684.1	,819.1,	778.4,	879.8	,857.2	,969.1	,898.45,	827.3,	854.1,	798.3	,851.2],
        [0	,155.63	,165.53,	119.15,	114.06	,120.17	,157.05,	169.24,	175.37	,165.11,	167.31	,155.89]
    ];
    public mounted() {
        this.echartsInit();
        PubSub.subscribe("energyChartZq", (msg, data) => { /* 接收弹窗事件,data为传输的数据 */
        if(data === '1'){
            console.log(data)
            this.show = true;
            this.monthNum = 0;
    ...
    public updateDataByMonth() {
        ...
        for(let i = 0; i < this.zqData.length; i++) {
            this.option.series[i].data = this.zqData[i].slice(0,this.monthNum);
        }
        this.echartsInit();
    }

    public destroyed() { // 关闭组件时取消订阅事件
        PubSub.unsubscribe("energyChartZq");
    }
